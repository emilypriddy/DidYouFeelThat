...
Parse.Cloud.beforeSave("USGSQuakes", function(request, response) {
  ...
  var detailURL = request.object.get("detailURL");
  var itemCountry = 
  Parse.Cloud.httpRequest({
  	...
  	if (quakeGeoserveProperties.country) {
  		request.object.set("country", quakeGeoserveProperties.country);
  	}
  	if (quakeGeoserveProperties.state) {
  		request.object.set("state", quakeGeoserveProperties.state);
  	}
  	console.log("Set country to: " + quakeGeoserveProperties.country + " and state to: " + quakeGeoserveProperties.state + " " + httpResponse.text);
