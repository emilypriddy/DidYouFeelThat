...
Parse.Cloud.beforeSave("GroupEvent", function(request, response) {
	var groupRelation = request.object.relation("relatedUserEvents");
	var groupEventLocation = request.object.get("eventLocation");

	var results = "FINISHED!!";
	var UserEvent = Parse.Object.extend("UserEvent");
	var userEventQuery = new Parse.Query(UserEvent);
	userEventQuery.withinMiles("userLocation", groupEventLocation, 100.0);
	userEventQuery.find({
		success: function(userEventObjs){
			for (i=0; i < userEventObjs.length; i++){
				...
				console.log("Inside GroupEvent.beforeSave - Found userEventObj.id: " + userEventObj.id);
		...
		error: function(error){
			console.log("Inside GroupEvent.beforeSave - Error finding nearby users! " + error.message);
		}
	}).then(function() {
	console.log("Inside GroupEvent.beforeSave - completed successfully!!");
	response.success();
}, function(error){
	response.error("Inside GroupEvent.beforeSave - Uh oh, something went wrong in GroupEvent.beforeSave! - " + error.message);
	});
});
