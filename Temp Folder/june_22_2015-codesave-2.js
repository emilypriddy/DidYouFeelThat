...
Parse.Cloud.afterSave("GroupEvent", function(request) {
	var groupEvent = request.object;
	var eventQuake = groupEvent.get("relatedQuake");
	var eventLocation = eventQuake.get("location");

	Parse.Cloud.run("findRelatedUserEvents", {
		groupEventId: groupEvent.id,
		eventQuakeId: eventQuake.id
	}, {
			success: function(result){
...
Parse.Cloud.define("findRelatedUserEvents", function(request, response) {
	...
	var eventQuakeId = request.params.eventQuakeId;

	var groupEvent, eventQuake;
	// var eventQuake = groupEvent.get("relatedQuake").
	// var eventLocation = eventQuake.get("location");

	var GroupEvent = Parse.Object.extend("GroupEvent");
	var query = new Parse.Query(GroupEvent);
	query.get(groupEventId, {
		success: function(result){
			console.log("inside findRelatedUserEvents successfully found groupEvent! - " + result.id);
			groupEvent = result;
		},
		error: function(myObj, error){
			console.log("error inside findRelatedUserEvents query.get(groupEventId) - " + error.message);
		}
	}).then(function(){
		var USGSQuakes = Parse.Object.extend("USGSQuakes");
		var query = new Parse.Query(USGSQuakes);
		query.get(eventQuakeId, {
			success: function(result){
				eventQuake = result;
				eventLocation = result.get("location");
				console.log("inside findRelatedUserEvents successfully found eventQuake! - " + result.id);
			},
			error: function(myObj, error){
				console.log("error inside findRelatedUserEvents query.get(eventQuakeId) - " + error.message);
			}
		});
	}).then(function() {
		console.log("eventLocation = " + eventLocation);
		response.success("findRelatedUserEvents completed successfully!!");
	}, function(error){
		response.error("Uh oh, something went wrong in findRelatedUserEvents! - " + error.message);
	});
});
